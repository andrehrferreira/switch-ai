...
- [ ] 1.5 `Pagination on requests`: Add `offset` parameter to `/api/requests` for proper pagination.
- [ ] 1.6 `Filtering on requests`: Add query params `?model=X&status=success&from=DATE&to=DATE` to `/api/requests`.
- [ ] 1.7 `CSV/JSON export`: `GET /api/requests/export?format=csv` and `format=json` for data export.
...
- [ ] 2.12 `Sidebar navigation`: Make sidebar links switch between views (Dashboard, Requests, Models, Blacklist, Backends).
- [ ] 2.13 `Requests view`: Dedicated full-page view with filters (model, status, date range), search, and pagination.
- [ ] 2.14 `Export button`: Button to download requests as CSV/JSON.
- [ ] 2.15 `Responsive design`: Ensure dashboard works on smaller screens.

## 3. Testing

- [ ] 3.1 `Dashboard API tests`: Unit tests for all endpoints in `src/server/dashboard-api.ts` (stats, requests, models, blacklist, categories, activity, backends).
- [ ] 3.2 `Control API tests`: Tests for `/api/debug` and `/api/backend` toggle endpoints.
- [ ] 3.3 `Server handler tests`: Tests for SSE streaming, non-streaming responses, and error handling in `src/server/index.ts`.
- [ ] 3.4 `Coverage`: Ensure test coverage remains >= 95%.
